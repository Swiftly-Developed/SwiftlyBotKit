...
enum LoginPage {
    ...
    static func render(error: String?, options: BotKitConfiguration.Dashboard = .default) -> String {
        let page = html(.lang("en")) {
            head {
                ...
                style { HTMLRaw(DashboardTheme.css + Self.css) }
            }
            body {
                main(.class("login")) {
                    div(.class("card")) {
    ...
    main.login .primary{width:100%;margin-top:18px;padding:10px;border-radius:8px;border:1px solid var(--text-primary);
      background:var(--text-primary);color:var(--surface-1);font:inherit;font-weight:650;cursor:pointer}
